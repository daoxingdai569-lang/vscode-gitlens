...
/** What a Pro/trial plan includes. */
const proPlanFeatures = [
	'Commit Graph & Visual History on private repos',
	'Unlimited cloud integrations',
	'AI features — 250K tokens/week',
	'Launchpad, Worktrees & Code Suggest',
];

/** What Advanced and above include; replaces the Pro list once the plan clears Advanced. */
const advancedPlanFeatures = [
	'Everything in Advanced',
	'Self-hosted integrations',
	'AI features — 1M tokens/week',
	'Centralized seat & policy management',
];
...
export class GlSettingsAccount extends SignalWatcher(LitElement) {
	...
	private renderPlanCard(sub: Subscription) {
		...
		const features =
			compareSubscriptionPlans(this.planId, 'advanced') >= 0 ? advancedPlanFeatures : proPlanFeatures;
	...
	private renderUpsell(sub: Subscription) {
		...
		return html`<div class="plan__upsell">
			<span class="plan__upsell-text">Advanced adds self-hosted integrations and 1M AI tokens/week.</span>
			<gl-button
				appearance="secondary"
	...
	/**
	 * Shown for any organization membership, not just multi-org accounts — but it names the ACTIVE
	 * org, so it still needs that name to render.
	 */
	private renderOrganizationCard(sub: Subscription) {
		...
		// Switching needs somewhere to switch TO, so a single-org account hides the action — and the
		// "you are in N others" tail with it, which would otherwise read "0 other organizations".
	...
	private renderFooter(sub: Subscription) {
		return html`<div class="footer">
			...
			>
			<a href=${urls.pricing}>Compare plans</a>
	...
	private getPlanFootnote(sub: Subscription): string {
		switch (sub.state) {
			case SubscriptionState.Paid:
				return sub.plan.actual.organizationId != null
					? 'Managed by your organization'
					: 'Plan managed on gitkraken.com';
